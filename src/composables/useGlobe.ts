...
export interface GlobeHandle {
  /** Kick off the scatter → converge animation. */
  start: () => void
  /** Tear down the WebGL context, listeners and observer. */
  dispose: () => void
}

const noopGlobe: GlobeHandle = { start: () => {}, dispose: () => {} }

/**
 * Mounts the dotted-Earth globe onto a canvas.
 * `canvasId` lets non-hero pages (e.g. About) reuse the identical animation.
 */
export function initHeroGlobe(
  ...
  camera.position.set(cameraX, 0, 6.8)
  ...
  const orbitDefs: [number, number, number][] = [
    [0x5be38b, 2.9, 0.4],
    [0xcbbe8a, 3.3, -0.55],
    [0x2fa66a, 2.6, 0.9],
  ]
    ...
    if (reduceMotion) {
      ...
      orbits.forEach((o) => {
        o.mesh.position.set(
          ...
          Math.sin(o.phase * 0.8 + o.tilt) * o.r * 0.4,
  ...
  // Route changes unmount the canvas; without this the WebGL context leaks and
  ...
  function dispose() {
    ...
    globeMat.dispose()
    sphGeo.dispose()
    orbits.forEach((o) => o.mat.dispose())
    renderer.dispose()
  }

  return { start: startGlobe, dispose }
}
...
export function initCtaGlobe(reduceMotion = false): void {
  ...
  c2.fillStyle = g
  c2.beginPath()
  c2.arc(32, 32, 32, 0, Math.PI * 2)
  c2.fill()

  const mat = new THREE.PointsMaterial({
    size: 0.055,
    map: new THREE.CanvasTexture(cn),
    vertexColors: true,
    transparent: true,
    opacity: 0,
    depthWrite: false,
