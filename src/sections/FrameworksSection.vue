  ...
  <section id="frameworks" class="relative py-20 md:py-28 overflow-hidden">
    <div
      ...
      style="
        width: 560px;
        height: 560px;
        background: radial-gradient(circle, rgba(58, 77, 184, 0.1), transparent 70%);
        top: -100px;
        left: -120px;
      "
    ></div>
    <div class="max-w-[1380px] mx-auto px-6 md:px-10 relative z-10">
      <div class="max-w-3xl mb-8">
        ...
        <h2
          ...
          data-reveal
        >
          What hits you, <span class="text-grad">and when.</span>
        </h2>
        <p class="text-graphite text-lg leading-relaxed" data-reveal>
          Companies face 40+ overlapping standards. Here is where every framework stands for you
          today — already in force, coming next, or always-on — and each one explained in plain
          language below.
        </p>
      </div>
      ...
      <div class="fw-board" data-reveal>
        <div class="fw-lane" style="--lc: #1d6b43">
          <div class="fw-lane-h">
            <i class="fw-pulse"></i><span class="font-display">In force today</span>
          </div>
          <div class="fw-lane-chips">
            <span
              ...
              :key="c.label"
              class="fw-chip"
              :class="{ 'fw-dim': isDimmed(c.rg) }"
              :data-rg="c.rg"
              :style="`--acc:${c.color}`"
              >{{ c.label }} <em>{{ c.sub }}</em></span
            >
          </div>
        </div>
      ...
      <div
        class="fw-list mt-10 pt-8 grid lg:grid-cols-2 gap-x-14"
        style="border-top: 1px solid var(--stone)"
        data-reveal
      >
        <div
          ...
          :key="fw.id"
          class="fw-row"
          :class="{ 'fw-dim': isDimmed(fw.category) }"
          :data-rg="fw.category"
          :style="`--acc:${fw.color}`"
        >
          ...
          <div>
            <div class="flex flex-wrap items-center gap-2 mb-1">
              <h3 class="font-display font-bold text-lg text-charcoal leading-snug">
                {{ fw.fullName }}
              </h3>
